// interactions/modals/orderSecondModal.js - Handles the second order creation modal (date inputs)
const logger = require('../../utils/logger');

/**
 * Handles the second order creation modal submission (with date inputs)
 * @param {Object} interaction - Discord interaction
 * @param {Object} client - Discord client
 */
async function handleSecondOrderModal(interaction, client) {
  try {
    ...
    if (!orderSession) {
      logger.error(`No session found for user ${userId} in second modal handling`);
    ...
    // Now start the skills selection process
    await startRoleSelection(interaction, client);
  } catch (error) {
    logger.error('Error handling second order modal:', error);
